...
def home():
	...
	# create a query in the url with name page and default value of 1 and with type int
...
def new_post():
	form = PostForm()
	if form.validate_on_submit():
		post = Post(title=form.title.data, content=form.content.data, user_id=current_user.id) # or author=current_user.id
		db.session.add(post)
		db.session.commit()
...
@app.route('/home/<int:post_id>') # expect an int
def post(post_id):
...
def user_posts(username):
	page = request.args.get('_page', 1, type=int) 
	user = User.query.filter_by(username=username).first_or_404() # get the username or 404 error
